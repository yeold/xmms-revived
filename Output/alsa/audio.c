...
int alsa_playing(void)
{
  ...
  /* Software-side: anything the feeder thread will still write out? */
  if (get_thread_buffer_filled() > hw_period_size_in)
    return TRUE;

  /* Hardware-side: drained when (almost) the whole PCM buffer is writable.
   * Do not use snd_pcm_delay() here: PipeWire's ALSA compatibility layer
   * never reports the delay reaching 0, so end-of-track was never detected. */
  state = snd_pcm_state(alsa_pcm);
  if (state != SND_PCM_STATE_RUNNING && state != SND_PCM_STATE_DRAINING)
    return FALSE;

  avail_bytes = snd_pcm_frames_to_bytes(alsa_pcm, alsa_get_avail());
  return avail_bytes < hw_buffer_size - hw_period_size;
}
...
static int alsa_setup(struct snd_format *f)
{
  ...
  if ((err = snd_pcm_open(&alsa_pcm, alsa_cfg.pcm_device, SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK)) < 0)
  {
    g_warning("alsa_setup(): Failed to open pcm device (%s): %s", alsa_cfg.pcm_device, snd_strerror(-err));
    alsa_pcm = NULL;
    g_free(outputf);
    outputf = NULL;
    return -1;
  ...
  if ((err = snd_pcm_hw_params_set_format(alsa_pcm, hwparams, outputf->format)) < 0)
  {
     ...
     */
    snd_pcm_format_t formats[] = {SND_PCM_FORMAT_S16_LE, SND_PCM_FORMAT_S16_BE, SND_PCM_FORMAT_U8};
    int i;
  ...
  if (outputf->channels != f->channels)
  {
    ...
    alsa_stereo_convert_func = xmms_convert_get_channel_func(outputf->xmms_format, outputf->channels, f->channels);
    if (alsa_stereo_convert_func == NULL)
      return -1;
  }
  ...
  if (outputf->rate != f->rate)
  {
    ...
    alsa_frequency_convert_func = xmms_convert_get_frequency_func(outputf->xmms_format, outputf->channels);
    if (alsa_frequency_convert_func == NULL)
      return -1;
  }
  ...
  if ((err = snd_pcm_hw_params_set_buffer_time_near(alsa_pcm, hwparams, &alsa_buffer_time, 0)) < 0)
  {
  ...
  if ((err = snd_pcm_hw_params_set_period_time_near(alsa_pcm, hwparams, &alsa_period_time, 0)) < 0)
  {
  ...
  if ((err = snd_pcm_sw_params_set_start_threshold(alsa_pcm, swparams, alsa_buffer_size - alsa_period_size) < 0))
  ...
  if (inputf->bps != outputf->bps)
  {
    ...
    hw_buffer_size_in = ((guint64)hw_buffer_size * inputf->bps + outputf->bps / 2) / outputf->bps;
    hw_period_size_in = ((guint64)hw_period_size * inputf->bps + outputf->bps / 2) / outputf->bps;
