...
static gint disk_open(AFormat fmt, gint rate, gint nch)
{
  gchar *filename, *title, *temp, *base;
  gint pos;
  ...
  base = title ? g_path_get_basename(title) : NULL;
  if (title == NULL || strlen(base) == 0)
  {
    g_free(base);
    g_free(title);
    ...
    if (title == NULL || strlen(base) == 0)
    {
      ...
      base = g_path_get_basename(title);
    }
  }
  filename = g_strdup_printf("%s/%s.wav", file_path, base);
  g_free(base);
  g_free(title);
...
static void path_browse_cb(GtkWidget *w, gpointer data)
{
  if (!path_dirbrowser)
  {
    path_dirbrowser = xmms_create_dir_browser(_("Select the directory where you want to store the output files:"),
...
static void disk_configure(void)
{
  ...
  if (!configure_win)
  {
    ...
    gtk_tooltips_set_tip(use_suffix_tooltips, use_suffix_toggle,
                         "If enabled, the extension from the original filename will not be stripped before adding the "
                         ".wav extension to the end.",
                         NULL);
    ...
    gtk_signal_connect_object(GTK_OBJECT(configure_cancel), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy),
