...
void real_draw_button(GdkWindow *w, Button *button)
{
  ...
  if (button->pressed)
    gdk_draw_pixmap(w, dock_gc, pixmap, button->pressed_x, button->pressed_y, button->x, button->y, button->width,
                    button->height);
  else
    gdk_draw_pixmap(w, dock_gc, pixmap, button->normal_x, button->normal_y, button->x, button->y, button->width,
                    button->height);
}
...
void real_draw_seekslider(GdkWindow *w)
{
  ...
  if (seekslider_visible)
  {
    ...
    if (seekslider_pos < SEEKSLIDER_MAX / 3)
      ...
      slider_x = 50;
    gdk_draw_pixmap(w, dock_gc, pixmap, slider_x, 112, SEEKSLIDER_X + seekslider_pos, SEEKSLIDER_Y, 3,
                    SEEKSLIDER_HEIGHT);
  }
  else
...
void button_press_cb(GtkWidget *w, GdkEventButton *event, gpointer data)
{
  ...
  if (xmms_running)
  {
    ...
    if (inside_region(event->x, event->y, SEEKSLIDER_X, SEEKSLIDER_Y, SEEKSLIDER_WIDTH, SEEKSLIDER_HEIGHT) &&
        seekslider_visible)
    {
...
void drag_data_received(GtkWidget *widget, GdkDragContext *context, int x, int y, GtkSelectionData *selection_data,
                        guint info, guint time)
{
  if (selection_data->data)
  {
    guchar *url = selection_data->data;
...
void init(void)
{
  ...
  gtk_widget_set_events(window,
                        GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_EXPOSURE_MASK);
  g_signal_connect(G_OBJECT(window), "expose_event", G_CALLBACK(expose_cb), NULL);
  g_signal_connect(G_OBJECT(window), "button_press_event", G_CALLBACK(button_press_cb), NULL);
  g_signal_connect(G_OBJECT(window), "button_release_event", G_CALLBACK(button_release_cb), NULL);
  g_signal_connect(G_OBJECT(window), "motion_notify_event", G_CALLBACK(motion_notify_cb), NULL);
  g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(destroy_cb), NULL);
  gtk_drag_dest_set(window, GTK_DEST_DEFAULT_ALL, drop_types, 1, GDK_ACTION_COPY);
  g_signal_connect(G_OBJECT(window), "drag_data_received", G_CALLBACK(drag_data_received), NULL);
  ...
  gtk_widget_set_events(icon_win,
                        GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_EXPOSURE_MASK);
  g_signal_connect(G_OBJECT(icon_win), "expose_event", G_CALLBACK(expose_cb), NULL);
  g_signal_connect(G_OBJECT(icon_win), "button_press_event", G_CALLBACK(button_press_cb), NULL);
  g_signal_connect(G_OBJECT(icon_win), "button_release_event", G_CALLBACK(button_release_cb), NULL);
  g_signal_connect(G_OBJECT(icon_win), "motion_notify_event", G_CALLBACK(motion_notify_cb), NULL);
  ...
  g_signal_connect(G_OBJECT(icon_win), "drag_data_received", G_CALLBACK(drag_data_received), NULL);
...
static struct option lopt[] = {{"help", 0, NULL, 'h'},    {"geometry", 1, NULL, 'g'}, {"session", 1, NULL, 's'},
                               {"command", 1, NULL, 'c'}, {"icon", 1, NULL, 'i'},     {"single", 0, NULL, 'n'},
                               {"title", 0, NULL, 't'},   {"version", 0, NULL, 'v'},  {NULL, 0, NULL, 0}};
