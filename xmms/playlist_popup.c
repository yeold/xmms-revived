...
static void playlist_popup_draw(PlaylistPopup *popup)
{
  ...
  skin_draw_pixmap(popup->window->window, popup->gc, SKIN_PLEDIT, popup->barx, popup->bary, 0, 0, 3,
                   popup->num_items * 18);
  for (i = 0; i < popup->num_items; i++)
  {
    if (i == popup->active)
      skin_draw_pixmap(popup->window->window, popup->gc, SKIN_PLEDIT, popup->sx[i], popup->sy[i], 3, i * 18, 22, 18);
    else
      skin_draw_pixmap(popup->window->window, popup->gc, SKIN_PLEDIT, popup->nx[i], popup->ny[i], 3, i * 18, 22, 18);
  }
...
void playlist_popup(gint x, gint y, gint num_items, gint *nx, gint *ny, gint *sx, gint *sy, gint barx, gint bary,
                    gint base, void (*handler)(gint item))
{
  if (popup)
    playlist_popup_destroy();
  ...
  g_signal_connect(G_OBJECT(popup->window), "expose_event", G_CALLBACK(playlist_popup_expose), NULL);
  g_signal_connect(G_OBJECT(popup->window), "motion_notify_event", G_CALLBACK(playlist_popup_motion), NULL);
  g_signal_connect(G_OBJECT(popup->window), "button_release_event", G_CALLBACK(playlist_popup_release), NULL);
  ...
  gdk_flush();
  playlist_popup_draw(popup);
  GdkGrabStatus st = gdk_pointer_grab(popup->window->window, FALSE, GDK_BUTTON_MOTION_MASK | GDK_BUTTON_RELEASE_MASK,
                                      NULL, NULL, GDK_CURRENT_TIME);

  if (st != GDK_GRAB_SUCCESS)
    g_warning("playlist_popup: pointer grab failed (%d)", st);
  gdk_flush();
}
