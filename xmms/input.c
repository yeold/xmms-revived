...
static void convert_to_s16_ne(AFormat fmt, gpointer ptr, gint16 *left, gint16 *right, int nch, int max)
{
  gint16 *ptr16;
  guint16 *ptru16;
  guint8 *ptru8;
...
void input_play(char *filename)
{
  ...
  {
    xmms_show_message(_("No output plugin"), _("No output plugin has been selected"), _("OK"), FALSE, NULL, NULL);
    mainwin_stop_pushed();
    return;
  }
...
static void input_general_file_info_box(char *filename, InputPlugin *ip)
{
  ...
  char *title, *iplugin, *base;

  base = g_path_get_basename(filename);
  window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_policy(GTK_WINDOW(window), FALSE, TRUE, FALSE);
  title = g_strdup_printf(_("File Info - %s"), base);
  ...
  g_signal_connect(window, "key-press-event", G_CALLBACK(util_dialog_keypress_cb), NULL);
