...
void playlist_list_draw(Widget *w)
{
  ...
  for (i = pl->pl_first; list && i < pl->pl_first + pl->pl_num_visible; list = list->next, i++)
  {
    ...
    if (entry->selected)
    {
      ...
      gdk_draw_rectangle(obj, gc, TRUE, pl->pl_widget.x, pl->pl_widget.y + ((i - pl->pl_first) * pl->pl_fheight), width,
                         pl->pl_fheight);
    }
    ...
    if (pos != -1 || entry->length != -1)
    {
      ...
      x = pl->pl_widget.x + width - gdk_text_width(playlist_list_font, tail, strlen(tail)) - 2;
