...
void prefswin_font_browse_cb(GtkWidget *w, gpointer data)
{
  ...
  gtk_font_selection_dialog_set_font_name(GTK_FONT_SELECTION_DIALOG(fontsel),
                                          gtk_entry_get_text(GTK_ENTRY(prefswin_options_font_entry)));
  g_signal_connect(G_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->ok_button), "clicked",
                   G_CALLBACK(prefswin_font_browse_ok), fontsel);
  g_signal_connect_swapped(G_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->cancel_button), "clicked",
                           G_CALLBACK(gtk_widget_destroy), G_OBJECT(fontsel));
...
void prefswin_mainwin_font_browse_cb(GtkWidget *w, gpointer data)
{
  ...
  if (!fontsel)
  {
    ...
    gtk_font_selection_dialog_set_font_name(GTK_FONT_SELECTION_DIALOG(fontsel),
                                            gtk_entry_get_text(GTK_ENTRY(prefswin_mainwin_font_entry)));
    g_signal_connect(G_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->ok_button), "clicked",
                     G_CALLBACK(prefswin_mainwin_font_browse_ok), fontsel);
    g_signal_connect_swapped(G_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->cancel_button), "clicked",
                             G_CALLBACK(gtk_widget_destroy), G_OBJECT(fontsel));
    g_signal_connect_swapped(G_OBJECT(fontsel), "destroy", G_CALLBACK(gtk_widget_destroyed), &fontsel);
    gtk_widget_show(fontsel);
...
void create_prefs_window(void)
{
  ...
  g_signal_connect(G_OBJECT(prefswin_eplugins_config), "clicked", G_CALLBACK(prefswin_econfigure),
                   prefswin_eplugins_list);
  ...
  g_signal_connect(G_OBJECT(prefswin_eplugins_use_cbox), "toggled", G_CALLBACK(prefswin_eplugins_use_cb),
                   prefswin_eplugins_list);
  ...
  g_signal_connect(G_OBJECT(prefswin_gplugins_config), "clicked", G_CALLBACK(prefswin_gconfigure),
                   prefswin_gplugins_list);
  ...
  g_signal_connect(G_OBJECT(prefswin_gplugins_use_cbox), "toggled", G_CALLBACK(prefswin_gplugins_use_cb),
                   prefswin_gplugins_list);
  ...
  g_signal_connect(G_OBJECT(prefswin_vplugins_config), "clicked", G_CALLBACK(prefswin_vconfigure),
                   prefswin_vplugins_list);
  ...
  g_signal_connect(G_OBJECT(prefswin_vplugins_use_cbox), "toggled", G_CALLBACK(prefswin_vplugins_use_cb),
                   prefswin_vplugins_list);
  ...
  gtk_tooltips_set_tip(prefswin_tooltips, options_giop, _("Read song title and length only when starting to play"),
                       NULL);
  ...
  gtk_tooltips_set_tip(prefswin_tooltips, options_giod,
                       _("Read song title and length when the song is visible in the playlist"), NULL);
  ...
  gtk_tooltips_set_tip(prefswin_tooltips, options_giol,
                       _("Read song title and length as soon as the song is loaded to the playlist"), NULL);
  ...
  prefswin_option_new_with_label_to_table(&cfg.allow_multiple_instances, _("Allow multiple instances"),
                                          GTK_TABLE(options_table), 1, 0);

  prefswin_option_new_with_label_to_table(&cfg.convert_twenty, _("Convert %20 to space"), GTK_TABLE(options_table), 0,
                                          1);
  ...
  gtk_tooltips_set_tip(prefswin_tooltips, opt,
                       _("The \"clutterbar\" is the row of buttons at the left side of the main window"), NULL);

  prefswin_option_new_with_label_to_table(&cfg.convert_underscore, _("Convert underscore to space"),
  ...
  prefswin_option_new_with_label_to_table(&cfg.dim_titlebar, _("Dim titlebar when inactive"), GTK_TABLE(options_table),
                                          0, 3);
  prefswin_option_new_with_label_to_table(&cfg.show_numbers_in_pl, _("Show numbers in playlist"),
                                          GTK_TABLE(options_table), 1, 3);

  prefswin_option_new_with_label_to_table(&cfg.sort_jump_to_file, _("Sort \"Jump to file\" alphabetically"),
                                          GTK_TABLE(options_table), 0, 4);
  prefswin_option_new_with_label_to_table(&cfg.eq_doublesize_linked, _("Equalizer doublesize linked"),
                                          GTK_TABLE(options_table), 1, 4);

  options_rt = prefswin_option_new_with_label_to_table(&cfg.use_realtime, _("Use realtime priority when available"),
  ...
  prefswin_option_new_with_label_to_table(&cfg.smooth_title_scroll, _("Smooth title scroll"), GTK_TABLE(options_table),
                                          1, 5);
  ...
  prefswin_option_new_with_label_to_table(&cfg.show_wm_decorations, _("Show window manager decorations"),
                                          GTK_TABLE(options_table), 0, 7);
  opt = prefswin_option_new_with_label_to_table(
      &cfg.use_backslash_as_dir_delimiter, _("Use \'\\\' as a directory delimiter"), GTK_TABLE(options_table), 1, 7);
  gtk_tooltips_set_tip(prefswin_tooltips, opt,
  ...
  opt = prefswin_option_new_with_label_to_table(&cfg.use_pl_metadata, _("Use meta-data in playlists"),
  ...
  prefswin_options_fontset =
      prefswin_option_new_with_label(&cfg.use_fontsets, _("Use fontsets (Enable for multi-byte charset support)"));
  ...
  g_signal_connect(G_OBJECT(prefswin_mainwin_font_browse), "clicked", G_CALLBACK(prefswin_mainwin_font_browse_cb),
                   NULL);
  ...
  prefswin_moreinfo_label = gtk_label_new(_("%0.2n - Display a 0 padded 2 char long tracknumber\n"
                                            "\n"
...
void gen_module_description(gchar *file, gchar *desc, gchar **full_desc)
{
  ...
  (*full_desc) = g_strdup_printf("%s   [%s]", desc, filename);
  g_free(filename);
}
...
void add_output_plugins(GtkOptionMenu *omenu)
{
  ...
  while (olist)
  {
    ...
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(prefswin_output_cb), GINT_TO_POINTER(i));
