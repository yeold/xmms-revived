...
void init_plugins(void)
{
  ...
  while (node)
  {
    ...
    gchar *cfgbase = g_path_get_basename(cfg.outputplugin);
    gchar *fname = g_path_get_basename(op->filename);

    if (!strcmp(cfgbase, fname))
...
static int plugin_check_duplicate(char *filename)
{
  ...
  for (l = ip_data->input_list; l && !found; l = l->next)
  {
    other_base = g_path_get_basename(((InputPlugin *)l->data)->filename);
    if (!strcmp(base_filename, other_base))
      found = 1;
    g_free(other_base);
  }

  for (l = op_data->output_list; l && !found; l = l->next)
  {
    other_base = g_path_get_basename(((OutputPlugin *)l->data)->filename);
    if (!strcmp(base_filename, other_base))
      found = 1;
    g_free(other_base);
  }

  for (l = ep_data->effect_list; l && !found; l = l->next)
  {
    other_base = g_path_get_basename(((EffectPlugin *)l->data)->filename);
    if (!strcmp(base_filename, other_base))
      found = 1;
    g_free(other_base);
  }

  for (l = gp_data->general_list; l && !found; l = l->next)
  {
    other_base = g_path_get_basename(((GeneralPlugin *)l->data)->filename);
    if (!strcmp(base_filename, other_base))
      found = 1;
    g_free(other_base);
  }

  for (l = vp_data->vis_list; l && !found; l = l->next)
  {
    other_base = g_path_get_basename(((VisPlugin *)l->data)->filename);
    if (!strcmp(base_filename, other_base))
      found = 1;
    g_free(other_base);
  }
