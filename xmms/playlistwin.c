...
GtkItemFactoryEntry playlistwin_sort_menu_entries[] = {
    ...
    {N_("/Sort List/By Title"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback, PLAYLISTWIN_SORT_BYTITLE,
     "<Item>"},
    {N_("/Sort List/By Filename"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_BYFILENAME, "<Item>"},
    {N_("/Sort List/By Path + Filename"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_BYPATH, "<Item>"},
    {N_("/Sort List/By Date"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback, PLAYLISTWIN_SORT_BYDATE,
     "<Item>"},
    {N_("/Selection"), NULL, NULL, 0, "<Branch>"},
    {N_("/Selection/Sort By Title"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_SEL_BYTITLE, "<Item>"},
    {N_("/Selection/Sort By Filename"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_SEL_BYFILENAME, "<Item>"},
    {N_("/Selection/Sort By Path + Filename"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_SEL_BYPATH, "<Item>"},
    {N_("/Selection/Sort By Date"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_SEL_BYDATE, "<Item>"},
    {N_("/Selection/Randomize"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback,
     PLAYLISTWIN_SORT_SEL_RANDOMIZE, "<Item>"},
    {"/-", NULL, NULL, 0, "<Separator>"},
    {N_("/Randomize List"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback, PLAYLISTWIN_SORT_RANDOMIZE,
     "<Item>"},
    {N_("/Reverse List"), NULL, (GtkItemFactoryCallback)playlistwin_sort_menu_callback, PLAYLISTWIN_SORT_REVERSE,
     "<Item>"},
};
...
enum
{
  PLAYLISTWIN_SAVE_EXTENSION = 0,
  PLAYLISTWIN_SAVE_M3U,
  PLAYLISTWIN_SAVE_PLS
};

static GtkItemFactoryEntry playlistwin_playlist_filetypes[] = {
    {N_("/By extension"), NULL, (GtkItemFactoryCallback)playlistwin_save_type_cb, PLAYLISTWIN_SAVE_EXTENSION, NULL},
    {"/-", NULL, NULL, 0, "<Separator>"},
    {"/m3u", NULL, (GtkItemFactoryCallback)playlistwin_save_type_cb, PLAYLISTWIN_SAVE_M3U, NULL},
    {"/pls", NULL, (GtkItemFactoryCallback)playlistwin_save_type_cb, PLAYLISTWIN_SAVE_PLS, NULL},
};
...
GtkItemFactoryEntry playlistwin_sub_menu_entries[] = {
    {N_("/Remove Dead Files"), NULL, (GtkItemFactoryCallback)playlistwin_sub_menu_callback,
     PLAYLISTWIN_REMOVE_DEAD_FILES, "<Item>"},
    {N_("/Physically Delete Files"), NULL, (GtkItemFactoryCallback)playlistwin_sub_menu_callback,
...
GtkItemFactoryEntry playlistwin_popup_menu_entries[] = {
    {N_("/View File Info"), "<control>3", (GtkItemFactoryCallback)playlistwin_popup_menu_callback, MISC_FILEINFO,
     "<Item>"},
    {N_("/Queue - Unqueue"), "Q", (GtkItemFactoryCallback)playlistwin_popup_menu_callback, MISC_QUEUE, "<Item>"},
    {N_("/Queue manager"), "<alt>Q", (GtkItemFactoryCallback)playlistwin_popup_menu_callback, MISC_QUEUE_MANAGER,
     "<Item>"},
    {N_("/-"), NULL, NULL, 0, "<Separator>"},
    {N_("/Add"), NULL, NULL, 0, "<Branch>"},
    {N_("/Add/File"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, ADD_FILE, "<Item>"},
    {N_("/Add/Directory"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, ADD_DIR, "<Item>"},
    {N_("/Add/Url"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, ADD_URL, "<Item>"},

    {N_("/Remove"), NULL, NULL, 0, "<Branch>"},
    {N_("/Remove/Selected"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SUB_SELECTED, "<Item>"},
    {N_("/Remove/Crop"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SUB_CROP, "<Item>"},
    {N_("/Remove/All"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SUB_ALL, "<Item>"},
    {N_("/Remove/Misc"), NULL, NULL, 0, "<Item>"},

    {N_("/Selection"), NULL, NULL, 0, "<Branch>"},
    {N_("/Selection/Select All"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SEL_ALL, "<Item>"},
    {N_("/Selection/Select None"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SEL_ZERO, "<Item>"},
    {N_("/Selection/Invert Selection"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SEL_INV,
     "<Item>"},
    {N_("/Selection/-"), NULL, NULL, 0, "<Separator>"},
    {N_("/Selection/Read Extended Info"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, SEL_LOOKUP,
     "<Item>"},

    {N_("/Sort"), NULL, NULL, 0, "<Item>"},

    {N_("/Playlist"), NULL, NULL, 0, "<Branch>"},
    {N_("/Playlist/Load List"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, PLIST_LOAD, "<Item>"},
    {N_("/Playlist/Save List"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, PLIST_SAVE, "<Item>"},
    {N_("/Playlist/New List"), NULL, (GtkItemFactoryCallback)playlistwin_popup_menu_callback, PLIST_NEW, "<Item>"},
};
...
static void playlistwin_update_info(void)
{
  ...
  {
    if (selection > 3600)
      sel_text = g_strdup_printf("%lu:%-2.2lu:%-2.2lu%s", selection / 3600, (selection / 60) % 60, selection % 60,
                                 (selection_more ? "+" : ""));
    else
  ...
  if (total > 0 || (total == 0 && !total_more))
  {
    if (total > 3600)
      tot_text =
          g_strdup_printf("%lu:%-2.2lu:%-2.2lu%s", total / 3600, (total / 60) % 60, total % 60, total_more ? "+" : "");
    else
...
static void playlistwin_resize(int width, int height)
{
  ...
  playlistwin_bg =
      gdk_pixmap_new(playlistwin->window, cfg.playlist_width, cfg.playlist_height, gdk_rgb_get_visual()->depth);
...
static void playlistwin_motion(GtkWidget *widget, GdkEventMotion *event, gpointer callback_data)
{
  ...
  if (playlistwin_resizing)
  {
    ...
    gdk_window_set_hints(playlistwin->window, 0, 0, cfg.playlist_width, PLAYLIST_HEIGHT, cfg.playlist_width,
                         PLAYLIST_HEIGHT, GDK_HINT_MIN_SIZE | GDK_HINT_MAX_SIZE);
...
static gboolean inside_sensitive_widgets(int x, int y)
{
  return (inside_widget(x, y, playlistwin_list) || inside_widget(x, y, playlistwin_slider) ||
          ...
          inside_widget(x, y, playlistwin_time_min) || inside_widget(x, y, playlistwin_time_sec) ||
          inside_widget(x, y, playlistwin_info) || inside_widget(x, y, playlistwin_vis) ||
          inside_widget(x, y, playlistwin_srew) || inside_widget(x, y, playlistwin_splay) ||
          inside_widget(x, y, playlistwin_spause) || inside_widget(x, y, playlistwin_sstop) ||
          inside_widget(x, y, playlistwin_sfwd) || inside_widget(x, y, playlistwin_seject) ||
          inside_widget(x, y, playlistwin_sscroll_up) || inside_widget(x, y, playlistwin_sscroll_down));
}
...
static void playlistwin_press(GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
  ...
  if (event->button == 1 && !cfg.show_wm_decorations &&
      ((!cfg.playlist_shaded && event->x > cfg.playlist_width - 20 && event->y > cfg.playlist_height - 20) ||
       (cfg.playlist_shaded && event->x >= cfg.playlist_width - 31 && event->x < cfg.playlist_width - 22)))
  {
  ...
  else if (event->button == 1 && REGION_L(12, 37, 29, 11))
  {
    ...
    playlist_popup(xpos + 12, ypos + cfg.playlist_height - (3 * 18) - 11, 3, nx, ny, sx, sy, barx, bary, ADD_URL,
                   playlistwin_popup_handler);
    grab = FALSE;
  }
  else if (event->button == 1 && REGION_L(41, 66, 29, 11))
  {
    ...
    playlist_popup(xpos + 41, ypos + cfg.playlist_height - (4 * 18) - 11, 4, nx, ny, sx, sy, barx, bary, SUB_MISC,
                   playlistwin_popup_handler);
    grab = FALSE;
  }
  else if (event->button == 1 && REGION_L(70, 95, 29, 11))
  {
    ...
    playlist_popup(xpos + 70, ypos + cfg.playlist_height - (3 * 18) - 11, 3, nx, ny, sx, sy, barx, bary, SEL_INV,
                   playlistwin_popup_handler);
    grab = FALSE;
  }
  else if (event->button == 1 && REGION_L(99, 124, 29, 11))
  {
    ...
    playlist_popup(xpos + 99, ypos + cfg.playlist_height - (3 * 18) - 11, 3, nx, ny, sx, sy, barx, bary, MISC_SORT,
                   playlistwin_popup_handler);
    grab = FALSE;
  }
  else if (event->button == 1 && REGION_R(46, 23, 29, 11))
  {
    ...
    playlist_popup(xpos + cfg.playlist_width - 46, ypos + cfg.playlist_height - (3 * 18) - 11, 3, nx, ny, sx, sy, barx,
                   bary, PLIST_NEW, playlistwin_popup_handler);
    grab = FALSE;
  }
  ...
  else if (event->button == 1 && event->type == GDK_BUTTON_PRESS && !inside_sensitive_widgets(event->x, event->y) &&
  ...
  else if (event->button == 1 && event->type == GDK_2BUTTON_PRESS && !inside_sensitive_widgets(event->x, event->y) &&
           event->y < 14)
  {
    playlistwin_set_shade(!cfg.playlist_shaded);
  ...
  else if (event->button == 3)
  {
    ...
    else
    {
      ...
      util_item_factory_popup_with_data(playlistwin_popup_menu, GINT_TO_POINTER(pos), NULL, event->x_root,
                                        event->y_root + 5, 3, event->time);
    }
    grab = FALSE;
  }
  ...
  if (grab)
    gdk_pointer_grab(playlistwin->window, FALSE, GDK_BUTTON_MOTION_MASK | GDK_BUTTON_RELEASE_MASK, NULL, NULL,
                     GDK_CURRENT_TIME);
}
...
static gboolean playlistwin_configure(GtkWidget *window, GdkEventConfigure *event, gpointer data)
{
  ...
  {
    if (event->width != cfg.playlist_width || (event->height != cfg.playlist_height && event->height != 14))
...
static void playlistwin_keypress_up_down_handler(PlayList_List *pl, gboolean up, guint state)
{
  ...
  if (pl->pl_prev_selected == -1 || (!playlistwin_item_visible(pl->pl_prev_selected) &&
...
static gboolean playlistwin_keypress(GtkWidget *w, GdkEventKey *event, gpointer data)
{
  ...
  case GDK_Up:
  case GDK_Down:
    playlistwin_keypress_up_down_handler(playlistwin_list, keyval == GDK_Up || keyval == GDK_KP_Up, event->state);
    break;
  case GDK_Page_Up:
  ...
  default:
    ...
    if (!gtk_accel_group_activate(playlistwin_accel, (GQuark)0, G_OBJECT(playlistwin), event->keyval, event->state))
      gtk_widget_event(mainwin, (GdkEvent *)event);
    refresh = FALSE;
    break;
  }
...
static void playlistwin_draw_frame(void)
{
  ...
  if (cfg.playlist_shaded)
  {
    ...
    skin_draw_pixmap(playlistwin_bg, playlistwin_gc, src, 99, (!playlistwin_focus && cfg.dim_titlebar) ? 57 : 42,
                     w - 50, 0, 50, 14);
  }
  else
  {
...
static void playlistwin_drag_data_received(GtkWidget *widget, GdkDragContext *context, gint x, gint y,
...
{
  playlistwin_sinfo = create_textbox(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, 4, 4, cfg.playlist_width - 35,
                                     FALSE, SKIN_TEXT);
  if (!cfg.playlist_shaded)
    hide_widget(playlistwin_sinfo);
  if (cfg.playlist_shaded)
    playlistwin_shade = create_pbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 21, 3,
                                       9, 9, 128, 45, 150, 42, playlistwin_shade_toggle, SKIN_PLEDIT);
  else
    playlistwin_shade = create_pbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 21, 3,
                                       9, 9, 157, 3, 62, 42, playlistwin_shade_toggle, SKIN_PLEDIT);
  playlistwin_shade->pb_allow_draw = FALSE;
  playlistwin_close = create_pbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 11, 3, 9,
                                     9, cfg.playlist_shaded ? 138 : 167, cfg.playlist_shaded ? 45 : 3, 52, 42,
                                     playlistwin_close_cb, SKIN_PLEDIT);
  playlistwin_close->pb_allow_draw = FALSE;
  playlistwin_list = create_playlist_list(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, 12, 20,
  ...
  playlistwin_slider = create_playlistslider(&playlistwin_wlist, playlistwin_bg, playlistwin_gc,
                                             cfg.playlist_width - 15, 20, cfg.playlist_height - 58, playlistwin_list);
  playlistwin_time_min = create_textbox(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 82,
                                        cfg.playlist_height - 15, 15, FALSE, SKIN_TEXT);
  playlistwin_time_sec = create_textbox(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 64,
                                        cfg.playlist_height - 15, 10, FALSE, SKIN_TEXT);
  playlistwin_info = create_textbox(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 143,
                                    cfg.playlist_height - 28, 85, FALSE, SKIN_TEXT);
  playlistwin_vis = create_vis(&playlistwin_wlist, playlistwin_bg, playlistwin->window, playlistwin_gc,
                               cfg.playlist_width - 223, cfg.playlist_height - 26, 72, FALSE);
  hide_widget(playlistwin_vis);

  playlistwin_srew = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 144,
                                    cfg.playlist_height - 16, 8, 7, playlist_prev);
  playlistwin_splay = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 138,
                                     cfg.playlist_height - 16, 10, 7, mainwin_play_pushed);
  playlistwin_spause = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 128,
                                      cfg.playlist_height - 16, 10, 7, input_pause);
  playlistwin_sstop = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 118,
                                     cfg.playlist_height - 16, 9, 7, mainwin_stop_pushed);
  playlistwin_sfwd = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 109,
                                    cfg.playlist_height - 16, 8, 7, playlist_next);
  playlistwin_seject = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 100,
                                      cfg.playlist_height - 16, 9, 7, mainwin_eject_pushed);
  playlistwin_sscroll_up = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 14,
                                          cfg.playlist_height - 35, 8, 5, playlistwin_scroll_up_pushed);
  playlistwin_sscroll_down = create_sbutton(&playlistwin_wlist, playlistwin_bg, playlistwin_gc, cfg.playlist_width - 14,
...
static void playlistwin_create_gtk(void)
{
  ...
  gtk_widget_set_events(playlistwin, GDK_FOCUS_CHANGE_MASK | GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
  ...
  g_signal_connect(G_OBJECT(playlistwin), "button_press_event", G_CALLBACK(playlistwin_press), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "button_release_event", G_CALLBACK(playlistwin_release), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "motion_notify_event", G_CALLBACK(playlistwin_motion), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "focus_in_event", G_CALLBACK(playlistwin_focus_in), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "focus_out_event", G_CALLBACK(playlistwin_focus_out), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "configure_event", G_CALLBACK(playlistwin_configure), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "client_event", G_CALLBACK(playlistwin_client_event), NULL);
  xmms_drag_dest_set(playlistwin);
  g_signal_connect(G_OBJECT(playlistwin), "drag-data-received", G_CALLBACK(playlistwin_drag_data_received), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "key-press-event", G_CALLBACK(playlistwin_keypress), NULL);
  g_signal_connect(G_OBJECT(playlistwin), "selection_received", G_CALLBACK(selection_received), NULL);
...
void playlistwin_create(void)
{
  ...
  gtk_item_factory_create_items(GTK_ITEM_FACTORY(playlistwin_sort_menu), playlistwin_sort_menu_entries_num,
  ...
  gtk_item_factory_create_items(GTK_ITEM_FACTORY(playlistwin_sub_menu), playlistwin_sub_menu_entries_num,
                                playlistwin_sub_menu_entries, NULL);
  playlistwin_bg = gdk_pixmap_new(NULL, cfg.playlist_width, cfg.playlist_height, gdk_rgb_get_visual()->depth);
  ...
  gtk_item_factory_create_items(GTK_ITEM_FACTORY(playlistwin_popup_menu), playlistwin_popup_menu_entries_num,
  ...
  gtk_item_factory_create_items(GTK_ITEM_FACTORY(playlistwin_save_menu), playlistwin_playlist_filetypes_num,
...
{
  if (!pposition_broken && cfg.playlist_x != -1 && cfg.save_window_position && cfg.show_wm_decorations)
    dock_set_uposition(playlistwin, cfg.playlist_x, cfg.playlist_y);
