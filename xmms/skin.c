...
static const gint skin_default_viscolor[24][3] = {
    {9, 34, 53},   {10, 18, 26},  {0, 54, 108},  {0, 58, 116},  {0, 62, 124},  {0, 66, 132},
    {0, 70, 140},  {0, 74, 148},  {0, 78, 156},  {0, 82, 164},  {0, 86, 172},  {0, 92, 184},
    {0, 98, 196},  {0, 104, 208}, {0, 110, 220}, {0, 116, 232}, {0, 122, 244}, {0, 128, 255},
    {0, 128, 255}, {0, 104, 208}, {0, 80, 160},  {0, 56, 112},  {0, 32, 64},   {200, 200, 200}};
...
GdkBitmap *skin_create_transparent_mask(const gchar *path, const gchar *file, const gchar *section, GdkWindow *window,
                                        gint width, gint height, gboolean doublesize)
{
  gchar *filename;
...
static void skin_numbers_generate_dash(SkinPixmap *numbers)
{
  ...
  pixmap = gdk_pixmap_new(mainwin->window, 108, numbers->current_height, gdk_rgb_get_visual()->depth);
...
static void skin_load_pixmaps(const char *path)
{
  ...
  skin->mask_eq_ds =
      skin_create_transparent_mask(path, "region.txt", "Equalizer", equalizerwin->window, 550, 232, TRUE);
  skin->mask_eq_shade =
      skin_create_transparent_mask(path, "region.txt", "EqualizerWS", equalizerwin->window, 275, 14, FALSE);
  skin->mask_eq_shade_ds =
      skin_create_transparent_mask(path, "region.txt", "EqualizerWS", equalizerwin->window, 550, 28, TRUE);
...
static char *skin_decompress_skin(const char *path)
{
  ...
  if ((ending = strrchr((const char *)path, '.')) == NULL)
    return NULL;
...
static void _load_skin(const gchar *path, gboolean force)
{
  ...
  if (path)
  {
    ...
    ending = strrchr(path, '.');
    if (ending && (!strcasecmp(ending, ".zip") || !strcasecmp(ending, ".wsz") || !strcasecmp(ending, ".tgz") ||
                   !strcasecmp(ending, ".gz") || !strcasecmp(ending, ".bz2") || !strcasecmp(ending, ".tar")))
    {
...
void skin_draw_pixmap(GdkDrawable *drawable, GdkGC *gc, SkinIndex si, gint xsrc, gint ysrc, gint xdest, gint ydest,
...
void skin_get_eq_spline_colors(guint32 (*colors)[19])
{
  ...
  if (skin->eqmain.pixmap != NULL && skin->eqmain.current_width >= 116 && skin->eqmain.current_height >= 313)
