...
GtkItemFactoryEntry equalizerwin_presets_menu_entries[] = {
    ...
    {N_("/Load/Preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_LOAD_PRESET,
     "<Item>"},
    {N_("/Load/Auto-load preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_LOAD_AUTOPRESET, "<Item>"},
    {N_("/Load/Default"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_LOAD_DEFAULT,
     "<Item>"},
    {N_("/Load/-"), NULL, NULL, 0, "<Separator>"},
    {N_("/Load/Zero"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_LOAD_ZERO,
     "<Item>"},
    {N_("/Load/-"), NULL, NULL, 0, "<Separator>"},
    {N_("/Load/From file"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_LOAD_FROM_FILE, "<Item>"},
    {N_("/Load/From WinAMP EQF file"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_LOAD_FROM_WINAMPFILE, "<Item>"},
    {N_("/Import"), NULL, NULL, 0, "<Branch>"},
    {N_("/Import/WinAMP Presets"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_IMPORT_WINAMPFILE, "<Item>"},
    {N_("/Save"), NULL, NULL, 0, "<Branch>"},
    {N_("/Save/Preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_SAVE_PRESET,
     "<Item>"},
    {N_("/Save/Auto-load preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_SAVE_AUTOPRESET, "<Item>"},
    {N_("/Save/Default"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_SAVE_DEFAULT,
     "<Item>"},
    {N_("/Save/-"), NULL, NULL, 0, "<Separator>"},
    {N_("/Save/To file"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_SAVE_TO_FILE,
     "<Item>"},
    {N_("/Save/To WinAMP EQF file"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_SAVE_TO_WINAMPFILE, "<Item>"},
    {N_("/Delete"), NULL, NULL, 0, "<Branch>"},
    {N_("/Delete/Preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb, EQUALIZER_PRESETS_DELETE_PRESET,
     "<Item>"},
    {N_("/Delete/Auto-load preset"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_DELETE_AUTOPRESET, "<Item>"},
    {N_("/Configure Equalizer"), NULL, (GtkItemFactoryCallback)equalizerwin_presets_menu_cb,
     EQUALIZER_PRESETS_CONFIGURE, "<Item>"},
};
...
void equalizerwin_set_shape_mask(void)
{
  ...
  gtk_widget_shape_combine_mask(equalizerwin, skin_get_mask(SKIN_MASK_EQ, EQUALIZER_DOUBLESIZE, cfg.equalizer_shaded),
                                0, 0);
}
...
void draw_equalizer_window(gboolean force)
{
  ...
  if (force)
  {
    ...
    {
      if (!cfg.equalizer_shaded)
        skin_draw_pixmap(equalizerwin_bg, equalizerwin_gc, SKIN_EQMAIN, 0, 134, 0, 0, 275, 14);
      else
    ...
    {
      if (!cfg.equalizer_shaded)
        skin_draw_pixmap(equalizerwin_bg, equalizerwin_gc, SKIN_EQMAIN, 0, 149, 0, 0, 275, 14);
      else
        skin_draw_pixmap(equalizerwin_bg, equalizerwin_gc, SKIN_EQ_EX, 0, 15, 0, 0, 275, 14);
    }
      ...
      else
      {
        ...
        while (wl)
        {
          ...
          if (w->redraw && w->visible)
          {
            ...
            img2 = create_dblsize_image(img);
            gdk_draw_image(equalizerwin_bg_dblsize, equalizerwin_gc, img2, 0, 0, w->x << 1, w->y << 1, w->width << 1,
                           w->height << 1);
            gdk_image_destroy(img2);
            gdk_image_destroy(img);
            w->redraw = FALSE;
...
{
  return (inside_widget(x, y, equalizerwin_on) || inside_widget(x, y, equalizerwin_auto) ||
...
void equalizerwin_press(GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
  ...
  if (event->button == 1 && event->type == GDK_BUTTON_PRESS &&
      ((cfg.easy_move || cfg.equalizer_shaded || event->y < 14) && !inside_sensitive_widgets(event->x, event->y)))
  {
  ...
  if (grab)
    gdk_pointer_grab(equalizerwin->window, FALSE, GDK_BUTTON_MOTION_MASK | GDK_BUTTON_RELEASE_MASK, NULL, NULL,
                     GDK_CURRENT_TIME);
}
...
gboolean equalizerwin_keypress(GtkWidget *w, GdkEventKey *event, gpointer data)
{
  ...
  default:
    if (!gtk_accel_group_activate(equalizerwin_accel,
                                  ...
                                  (GQuark)0, G_OBJECT(equalizerwin), event->keyval, event->state))
      gtk_widget_event(mainwin, (GdkEvent *)event);
    break;
  }
...
static void equalizerwin_create_widgets(void)
{
  ...
  equalizerwin_on = create_tbutton(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 14, 18, 25, 12, 10, 119, 128,
                                   119, 69, 119, 187, 119, equalizerwin_on_pushed, SKIN_EQMAIN);
  tbutton_set_toggled(equalizerwin_on, cfg.equalizer_active);
  equalizerwin_auto = create_tbutton(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 39, 18, 33, 12, 35, 119,
                                     153, 119, 94, 119, 212, 119, equalizerwin_auto_pushed, SKIN_EQMAIN);
  tbutton_set_toggled(equalizerwin_auto, cfg.equalizer_autoload);
  equalizerwin_presets = create_pbutton(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 217, 18, 44, 12, 224,
                                        164, 224, 176, equalizerwin_presets_pushed, SKIN_EQMAIN);
  equalizerwin_close = create_pbutton(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 264, 3, 9, 9, 0, 116, 0,
                                      125, equalizerwin_close_cb, SKIN_EQMAIN);
  equalizerwin_close->pb_allow_draw = FALSE;

  equalizerwin_shade = create_pbutton_ex(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 254, 3, 9, 9, 254, 137,
                                         1, 38, equalizerwin_shade_toggle, SKIN_EQMAIN, SKIN_EQ_EX);
  equalizerwin_shade->pb_allow_draw = FALSE;
  ...
  equalizerwin_volume = create_hslider(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 61, 4, 97, 8, 1, 30, 1,
                                       30, 3, 7, 4, 61, 0, 94, equalizerwin_volume_frame_cb,
                                       equalizerwin_volume_motion_cb, equalizerwin_volume_release_cb, SKIN_EQ_EX);
  equalizerwin_balance = create_hslider(&equalizerwin_wlist, equalizerwin_bg, equalizerwin_gc, 164, 4, 42, 8, 11, 30,
...
static void equalizerwin_create_gtk(void)
{
  ...
  gtk_widget_set_events(equalizerwin, GDK_FOCUS_CHANGE_MASK | GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
  ...
  g_signal_connect(G_OBJECT(equalizerwin), "button_press_event", G_CALLBACK(equalizerwin_press), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "button_release_event", G_CALLBACK(equalizerwin_release), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "motion_notify_event", G_CALLBACK(equalizerwin_motion), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "focus_in_event", G_CALLBACK(equalizerwin_focus_in), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "focus_out_event", G_CALLBACK(equalizerwin_focus_out), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "configure_event", G_CALLBACK(equalizerwin_configure), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "client_event", G_CALLBACK(equalizerwin_client_event), NULL);
  g_signal_connect(G_OBJECT(equalizerwin), "key-press-event", G_CALLBACK(equalizerwin_keypress), NULL);
}
...
void equalizerwin_show(gboolean show)
{
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(gtk_item_factory_get_widget(mainwin_general_menu, "/Graphical EQ")), show);
}
...
void equalizerwin_real_show(void)
{
   ...
   */
  if (!pposition_broken && cfg.equalizer_x != -1 && cfg.save_window_position && cfg.show_wm_decorations)
    dock_set_uposition(equalizerwin, cfg.equalizer_x, cfg.equalizer_y);
...
static void equalizerwin_load_auto_select(GtkWidget *widget, gint row, gint column, GdkEventButton *event,
                                          gpointer data)
{
  if (event && event->type == GDK_2BUTTON_PRESS)
...
static GtkWidget *equalizerwin_create_list_window(GList *preset_list, gchar *title, GtkWidget **window,
                                                  GtkSelectionMode sel_mode, GtkWidget **entry, gchar *btn1_caption,
                                                  gchar *btn2_caption, GCallback btn1_func, GCallback select_row_func)
{
  ...
  *window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  ...
  if (select_row_func)
    g_signal_connect_swapped(G_OBJECT(clist), "select_row", G_CALLBACK(select_row_func), NULL);
...
void equalizerwin_presets_menu_cb(gpointer cb_data, guint action, GtkWidget *w)
{
  ...
  case EQUALIZER_PRESETS_LOAD_PRESET:
    if (!equalizerwin_load_window)
      equalizerwin_create_list_window(equalizer_presets, _("Load preset"), &equalizerwin_load_window,
                                      GTK_SELECTION_SINGLE, NULL, _("OK"), _("Cancel"),
                                      G_CALLBACK(equalizerwin_load_ok), G_CALLBACK(equalizerwin_load_select));
    else
      gdk_window_raise(equalizerwin_load_window->window);
    break;
  case EQUALIZER_PRESETS_LOAD_AUTOPRESET:
    if (!equalizerwin_load_auto_window)
      equalizerwin_create_list_window(equalizer_auto_presets, _("Load auto-preset"), &equalizerwin_load_auto_window,
                                      GTK_SELECTION_SINGLE, NULL, _("OK"), _("Cancel"),
                                      G_CALLBACK(equalizerwin_load_auto_ok), G_CALLBACK(equalizerwin_load_auto_select));
    else
  ...
  case EQUALIZER_PRESETS_SAVE_PRESET:
    if (!equalizerwin_save_window)
      equalizerwin_create_list_window(equalizer_presets, _("Save preset"), &equalizerwin_save_window,
                                      GTK_SELECTION_SINGLE, &equalizerwin_save_entry, _("OK"), _("Cancel"),
                                      G_CALLBACK(equalizerwin_save_ok), G_CALLBACK(equalizerwin_save_select));
    else
  ...
  case EQUALIZER_PRESETS_SAVE_AUTOPRESET:
  {
    ...
    if (!equalizerwin_save_auto_window)
      equalizerwin_create_list_window(equalizer_auto_presets, _("Save auto-preset"), &equalizerwin_save_auto_window,
                                      GTK_SELECTION_SINGLE, &equalizerwin_save_auto_entry, _("OK"), _("Cancel"),
                                      G_CALLBACK(equalizerwin_save_auto_ok), G_CALLBACK(equalizerwin_save_auto_select));
    else
    ...
    if ((name = playlist_get_filename(get_playlist_position())) != NULL)
    {
      ...
      gtk_entry_set_text(GTK_ENTRY(equalizerwin_save_auto_entry), base);
      g_free(base);
      g_free(name);
    }
    break;
  ...
  case EQUALIZER_PRESETS_DELETE_PRESET:
    if (!equalizerwin_delete_window)
      equalizerwin_create_list_window(equalizer_presets, _("Delete preset"), &equalizerwin_delete_window,
  ...
  case EQUALIZER_PRESETS_DELETE_AUTOPRESET:
    if (!equalizerwin_delete_auto_window)
      equalizerwin_create_list_window(equalizer_auto_presets, _("Delete auto-preset"), &equalizerwin_delete_auto_window,
                                      GTK_SELECTION_EXTENDED, NULL, _("Delete"), _("Close"),
...
void equalizerwin_load_auto_preset(gchar *filename)
{
   ...
   */
  if (strlen(cfg.eqpreset_extension) > 0 && (cfgfile = xmms_cfg_open_file(presetfilename)) != NULL)
  {
  ...
  if (strlen(cfg.eqpreset_default_file) > 0 && (cfgfile = xmms_cfg_open_file(presetfilename)) != NULL)
  {
    equalizerwin_read_xmms_preset(cfgfile);
    xmms_cfg_free(cfgfile);
  }
