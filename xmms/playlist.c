...
void playlist_delete_node(GList *node, gboolean *set_info_text, gboolean *restart_playing)
{
  ...
  else if (playing_song && g_list_position(playlist, playing_song) > g_list_position(playlist, node))
    *set_info_text = TRUE;
...
char *playlist_get_info_text(void)
{
  ...
  if (playlist_position->length != -1)
    length =
        g_strdup_printf(" (%d:%-2.2d)", playlist_position->length / 60000, (playlist_position->length / 1000) % 60);
  else
    length = g_strdup("");
...
static void *playlist_get_info_func(void *arg)
{
  ...
  while (playlist_get_info_going)
  {
    ...
    else if (!cfg.get_info_on_load && cfg.get_info_on_demand && cfg.playlist_visible && !cfg.playlist_shaded)
    {
      ...
      for (node = g_list_nth(get_playlist(), playlistwin_get_toprow());
           node && playlistwin_item_visible(g_list_position(get_playlist(), node)); node = g_list_next(node))
      {
        entry = node->data;
        if (entry->title || entry->length != -1)
          continue;
