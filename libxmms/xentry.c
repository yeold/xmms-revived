...
static int gtk_entry_key_press(GtkWidget *widget, GdkEventKey *event)
{
  ...
  // has_selection =
  gtk_editable_get_selection_bounds(editable, (gint *)&sel_start_pos, (gint *)&sel_end_pos);
  ...
  case GDK_Left:
    ...
    {
      gtk_editable_set_position(editable, MIN(sel_start_pos, sel_end_pos));
      /* Force redraw below */
      initial_pos = -1;
    }
    else
  ...
  case GDK_Right:
    ...
    if (!extend_selection && sel_start_pos != sel_end_pos)
    {
      gtk_editable_set_position(editable, MAX(sel_start_pos, sel_end_pos));
      /* Force redraw below */
      initial_pos = -1;
    }
    else
