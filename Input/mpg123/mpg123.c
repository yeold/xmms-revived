...
static void set_synth_functions(struct frame *fr)
{
  ...
  static func funcs[][3] = {{mpg123_synth_1to1, mpg123_synth_2to1, mpg123_synth_4to1},
                            {mpg123_synth_1to1_8bit, mpg123_synth_2to1_8bit, mpg123_synth_4to1_8bit},
#ifdef USE_SIMD
  ...
  static func_mono funcs_mono[2][4] = {
      {mpg123_synth_1to1_mono, mpg123_synth_2to1_mono, mpg123_synth_4to1_mono},
      {mpg123_synth_1to1_8bit_mono, mpg123_synth_2to1_8bit_mono, mpg123_synth_4to1_8bit_mono}};
  ...
  if (CPU_HAS_3DNOW() && !p8 && (mpg123_cfg.default_synth == SYNTH_3DNOW || mpg123_cfg.default_synth == SYNTH_AUTO))
  {
    fr->synth = funcs[3][ds];   /* 3DNow! optimized synth_1to1() */
    fr->dct36 = funcs_dct36[1]; /* 3DNow! optimized dct36() */
    fr->synth_type = SYNTH_3DNOW;
  }
  else if (CPU_HAS_MMX() && !p8 && (mpg123_cfg.default_synth == SYNTH_MMX || mpg123_cfg.default_synth == SYNTH_AUTO))
  {
  ...
  { /* We assume all http:// (except those ending in .ogg) are mpeg -- why do we do that? */
    ...
    if (ext)
    {
      if (!strncasecmp(ext, ".ogg", 4))
        return FALSE;
      if (!strncasecmp(ext, ".rm", 3) || !strncasecmp(ext, ".ra", 3) || !strncasecmp(ext, ".rpm", 4) ||
          !strncasecmp(ext, ".fla", 4) || !strncasecmp(ext, ".flac", 5) || !strncasecmp(ext, ".ram", 4))
        return FALSE;
    }
    return TRUE;
  }
...
static gchar *get_song_title(FILE *fd, char *filename)
{
  ...
  if (file || (file = fopen(filename, "rb")) != 0)
  {
    ...
    if (!id3 && (fseek(file, -1 * sizeof(id3v1tag), SEEK_END) == 0) &&
        (fread(&id3v1tag, 1, sizeof(id3v1tag), file) == sizeof(id3v1tag)) && (strncmp(id3v1tag.tag, "TAG", 3) == 0))
    {
    ...
    if (mpg123_head_check(h2) && mpg123_decode_header(&tmp, h2) && (h & mask) == (h2 & mask))
    {
      ...
      if (buffer)
      {
        *buffer = g_malloc(offset + 4);
        if (fread(*buffer, 1, offset + 4, fh) != offset + 4 || fseek(fh, -(offset + 4), SEEK_CUR))
        {
          g_free(*buffer);
          return FALSE;
        }
...
static void *decode_loop(void *arg)
{
  ...
  if (!mpg123_info->eof && mpg123_info->going)
  {
    ...
    for (;;)
    {
      ...
      if (fr.lay != temp_fr.lay || fr.sampling_frequency != temp_fr.sampling_frequency || fr.stereo != temp_fr.stereo ||
          fr.lsf != temp_fr.lsf)
    ...
    mpg123_ip.set_info(mpg123_title, mpg123_length, mpg123_bitrate * 1000, mpg123_freqs[fr.sampling_frequency],
                       fr.stereo);
    output_opened = TRUE;
    if (!open_output(fr))
    {
...
static void aboutbox(void)
{
  ...
  gtk_signal_connect(GTK_OBJECT(aboutbox), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &aboutbox);
}
