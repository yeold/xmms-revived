...
static void save_cb(GtkWidget *w, gpointer data)
{
  ...
  if ((fd = open(current_filename, O_RDWR)) != -1)
  {
    ...
    if (write(fd, &tag, sizeof(tag)) != sizeof(tag))
      msg = g_strdup_printf(_("%s\nUnable to write to file: %s"), _("Couldn't write tag!"), strerror(errno));
    close(fd);
  }
  else
    msg = g_strdup_printf(_("%s\nUnable to open file: %s"), _("Couldn't write tag!"), strerror(errno));
  if (msg)
  {
...
static void remove_id3_cb(GtkWidget *w, gpointer data)
{
  ...
  if ((fd = open(current_filename, O_RDWR)) != -1)
  {
    ...
    msg = g_strdup_printf(_("%s\nUnable to open file: %s"), _("Couldn't remove tag!"), strerror(errno));
  if (msg)
  {
    GtkWidget *mwin = xmms_show_message(_("File Info"), msg, _("OK"), FALSE, NULL, NULL);
    gtk_window_set_transient_for(GTK_WINDOW(mwin), GTK_WINDOW(window));
...
{
  static const char *channel_mode[] = {N_("Stereo"), N_("Joint stereo"), N_("Dual channel"), N_("Single channel")};
  if (mode < 0 || mode > 3)
    return "";
...
void mpg123_file_info_box(char *filename)
{
  ...
  if (!window)
  {
    ...
    g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(file_info_box_keypress_cb), NULL);
    ...
    gtk_table_attach(GTK_TABLE(table), artist_entry, 1, 4, 1, 2, GTK_FILL | GTK_EXPAND | GTK_SHRINK,
                     GTK_FILL | GTK_EXPAND | GTK_SHRINK, 0, 5);
    ...
    gtk_table_attach(GTK_TABLE(table), comment_entry, 1, 4, 3, 4, GTK_FILL | GTK_EXPAND | GTK_SHRINK,
                     GTK_FILL | GTK_EXPAND | GTK_SHRINK, 0, 5);
    ...
    gtk_table_attach(GTK_TABLE(table), year_entry, 1, 2, 4, 5, GTK_FILL | GTK_EXPAND | GTK_SHRINK,
                     GTK_FILL | GTK_EXPAND | GTK_SHRINK, 0, 5);
    ...
    gtk_table_attach(GTK_TABLE(table), tracknum_entry, 3, 4, 4, 5, GTK_FILL | GTK_EXPAND | GTK_SHRINK,
                     GTK_FILL | GTK_EXPAND | GTK_SHRINK, 0, 5);
    ...
    g_signal_connect(G_OBJECT(GTK_COMBO(genre_combo)->list), "select-child", G_CALLBACK(genre_selected), NULL);
  ...
  gtk_list_select_item(GTK_LIST(GTK_COMBO(genre_combo)->list), genre_find_index(genre_list, 0xff));
  ...
  if ((fh = fopen(current_filename, "rb")) != NULL)
  {
    ...
    label_set_text(mpeg_samplerate, _("Samplerate: %d Hz"), mpg123_freqs[frm.sampling_frequency]);
