...
static unsigned int convert(void *data, int in)
{
  ...
  else if (wav_file->format_tag == WAVE_FORMAT_ALAW || wav_file->format_tag == WAVE_FORMAT_MULAW)
  {
    int i;
    guint8 *src = data;
    guint16 *dst = data;
...
static void *play_loop(void *arg)
{
  ...
  data = g_malloc(512 * (MAX(wav_file->bits_per_sample, wav_file->bits_per_sample_out) / 8) * wav_file->channels);
  while (wav_file->going)
  {
    if (!wav_file->eof)
    {
      ...
      if (input > 0 && (actual_read = fread(data, 1, input, wav_file->file)) > 0)
      {
        ...
        wav_ip.add_vis_pcm(wav_ip.output->written_time(), wav_file->afmt, wav_file->channels, output, data);
        while (wav_ip.output->buffer_free() < output && wav_file->going && wav_file->seek_to == -1)
          xmms_usleep(10000);
...
static struct wave_file *construct_wave_file(char *filename)
{
  ...
  {
    if (len < 24 || fseek(wav->file, 8, SEEK_CUR) || !read_le_short(wav->file, &wav->format_tag))
      goto exit_on_error;
    len -= 10;
  }
...
static void play_file(char *filename)
{
  ...
  if (wav_ip.output->open_audio(wav_file->afmt, wav_file->samples_per_sec, wav_file->channels) == 0)
  {
    audio_error = TRUE;
    fclose(wav_file->file);
    g_free(wav_file);
...
static void get_song_info(char *filename, char **title, int *length)
{
  ...
  *length = 1000 * (wav->length / (wav->samples_per_sec * wav->channels * (wav->bits_per_sample / 8)));
...
static InputPlugin wav_ip = {
    NULL, NULL, NULL,     plugin_init, NULL, NULL, is_our_file, NULL, play_file, stop, wav_pause,
