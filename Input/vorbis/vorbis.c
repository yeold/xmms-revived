...
static void *vorbis_play_loop(void *arg)
{
  ...
  while (vorbis_playing)
  {
    ...
    if (!(vi->bitrate_upper == vi->bitrate_lower == vi->bitrate_nominal) &&
        (vorbis_ip.output->output_time() > timercount + 1000 || vorbis_ip.output->output_time() < timercount))
    {
      /*
...
static char *vorbis_generate_title(OggVorbis_File *vorbisfile, char *fn)
{
  ...
  if (temp)
    *temp = '\0';
  input->file_name = g_path_get_basename(fn);
  ...
  displaytitle =
      xmms_get_titlestring(vorbis_cfg.tag_override ? vorbis_cfg.tag_format : xmms_get_gentitle_format(), input);
